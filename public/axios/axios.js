...
editbtnAddress.addEventListener("click", () => {
  ...
  axios
    .put("/customer-setting", editAddressData)
    .then((data) => {
      //    console.log("this is address hello",data.data);
      (document.querySelector("#customerAddressB-words").innerHTML =
        data.data.building_address),
        (document.querySelector("#customerAddressSA-words").innerHTML =
          data.data.street_address),
        (document.querySelector("#customerAddressC-words").innerHTML =
          data.data.country_address);
